...
import {Navigate, resolvePath, Route, Routes, useLocation} from 'react-router';
...
class App extends React.Component {
    constructor(props) {
        super(props);
        window.notifyStatus = () => {
            window.activity.trackStatus(this.props.store?.status)
        };
    ...
    restoreDestination = () => {
        if (this.state.firstTime && this.state.location) {
            this.state.firstTime = false;
            return <Navigate to={this.state.location}/>
        }
    ...
    saveState = () => {
        if (this.props.store?.status === gameStates.paused) {
            let state = {};
            state.location = "/game" //saving location todo:
    ...
    restoreState = () => {
        ...
        if (typeof stateString === "string" && stateString.length > 0) {
            ...
            this.props.dispatch({
                type: "share",
                payload: {...(state.store), groundDimension: document.body.getBoundingClientRect()}
            });//restoring state, updating values needing to be updated
